...
- Absolutely no `ObservableObject` usage. Everything must be done wih
  `@Observable`.
...
- The Garmin FITSwiftSDK interface is a dependency, so the app should not need
  to implmeent its own FIT processing code (other than special metrics).
  https://github.com/garmin/fit-swift-sdk.git - this is available in the
  DerivedData folder, e.g. /Users/raj/Library/Developer/Xcode/DerivedData/FitExplore-actngjpipgbuqxcnadavzbgobums/SourcePackages/checkouts/fit-swift-sdk
